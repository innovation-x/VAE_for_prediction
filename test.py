...
def write_video(video, name, fps):
    # get the shape of video
    ...
    vid_arr = torch.permute(video_as_uint8, (1, 2, 3, 0))
    vid_arr = vid_arr.cpu()
    write_video(name, vid_arr, fps=fps)
...
if __name__ == '__main__':
    ...
    video_filename = os.path.join(abs_dir, 'datasets/mobileRobo/test/Library_vid/1/1.mp4')
    ...
    vae.load_state_dict(torch.load(checkpoint_path + '/m97.pth.tar'))
    vae = vae.module.eval()
    ...
    mu, log_sigma = vae.encode(video)
    sigma = log_sigma.exp()
    epsilon = torch.randn_like(sigma)
    z_reparametrized = mu + sigma * epsilon
    # z_reparametrized = torch.randn(1, args.z_dim, 4, 16, 16).to(device)
    print(z_reparametrized.shape)
    video_recon = vae.decode(z_reparametrized)

    write_video(video_recon, 'demo.mp4', fps=10)
    write_video(video, 'origin.mp4', fps=10)
